...
export default function SeparatePage() {
  ...
  // googleApis video, working when crossOrigin is set
  const sourceURL1 =
    "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ElephantsDream.mp4";
  // Expensify cloudfront video, audio not working
  const sourceURL2 =
    "https://d2k5nsl2zxldvw.cloudfront.net/videos/intro-1280.mp4";
  // Same as above but with encryptedAuthToken, doesn't change a thing
  const sourceURL3 =
  ...
  // blazejkustra's cloudfront mock video, audio didn't work on server's default settings
  // (just like sources 2 and 3), enabling CORS reading OPTIONS made audio work (works now)
  const sourceURL4 = "https://d3snkl3dd5j6bx.cloudfront.net/xd.mp4";
  // AV1 codec video, breaking on some platforms
  const sourceURL5 =
  ...
  return (
    ...
    >
      ...
      />
      {status === "readyToPlay" && (
        <Text style={{ color: "green" }}>EVERYTHING OK</Text>
      )}
      {status === "error" && (
        <Text style={{ color: "red" }}>THERE IS AN ERROR</Text>
      )}
      <Pressable
        onPressIn={enterFullscreen}
        style={{ backgroundColor: "magenta", width: 50, height: 50 }}
      >
        <Text>Fullscreen</Text>
      </Pressable>
      <Pressable
        onPressIn={() => videoPlayer.play()}
